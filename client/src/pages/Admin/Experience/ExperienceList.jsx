    ...
    const handleDragEnd = async (event) => {
        ...
        if (active.id !== over.id) {
            setExperiences((items) => {
                ...
                // Update order in database (fire and forget / optimistic UI)
                const updates = newItems.map((item, index) => ({
                    _id: item._id,
                    order: index
                }));

                api.put('/experience/reorder', { experiences: updates })
                    ...
                    <div className="space-y-4">
                        {experiences.map(exp => (
                            <SortableItem key={exp._id} id={exp._id}>
                                <div className="bg-gray-800 p-4 rounded-lg flex justify-between items-center group cursor-grab active:cursor-grabbing">
