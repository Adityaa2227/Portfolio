        ...
        <div className="glass-panel p-8 rounded-xl mb-10 border border-primary/30 relative overflow-hidden">
             ...
             {activeResume ? (
                 <div className="flex items-center gap-6">
                     ...
                     <a href={`http://localhost:5000${activeResume.fileUrl}`} target="_blank" className="text-blue-400 hover:underline">View PDF</a>
                 </div>
             ) : (
                ...
                {history.map(item => (
                    <div key={item._id} className={`glass-card p-4 rounded-lg flex justify-between items-center ${item.isActive ? 'border-l-4 border-l-primary' : 'opacity-60'}`}>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <a href={`http://localhost:5000${item.fileUrl}`} target="_blank" className="text-gray-400 hover:text-white"><Clock size={16}/></a>
                        </div>
                    </div>
                ))}
            </div>
